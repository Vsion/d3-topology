...
$, win, ready = {
    ...
    getPath: function(){
        var js = document.scripts, jsPath;
        for(var i = js.length-1;i>-1;i--){
            if(js[i].src.indexOf("layer.js")>0){
                jsPath = js[i].src;
                break;
            }
        }
...
if(!window.layer){//蛋疼的layer连单例处理都没有 add by hbf 16/3/23
//默认内置方法。
window.layer = {
    v: '1.8.5',
    ...
    confirm: function(msg, yes, fn, no){
        var isfn = (typeof fn === 'function'), conf = {
            dialog: {msg: msg, type: 4, yes: yes, no: isfn ? fn : no}
        };
        isfn || (conf.title = fn);
        return $.layer(conf);
    },
...
var page_padding_lr = 7,page_padding_tb = 9,xubox_title_h = 32,xubox_botton_h = 44;
...
Class.pt.config = {
    ...
    zIndex: 19891014,
    maxWidth: 400,
    dialog: { btn: ['确定','取消'], type : 8, msg: '', yes: function(index){layer.close(index);}, no: function(index){layer.close(index);},extend1: function(index){layer.close(index);},extend2: function(index){layer.close(index);},extend3: function(index){layer.close(index);},},
...
Class.pt.space = function(html){
    ...
    var ico = dialog.type === -1 ? '' : '<span class="xubox_msg xulayer_png32 xubox_msgico xubox_msgtype' + dialog.type + '"></span>',
    frame = [
    '<div class="xubox_dialog">'+ ico +'<span class="xubox_msg xubox_text" style="'+ (ico ? '' : 'padding-left:20px') +'">' + dialog.msg + '</span></div>',
    '<div class="xubox_page" style="overflow:auto;">'+ html +'</div>',
    ...
    config.shade[0] && (shade = '<div times="'+ times +'" id="xubox_shade' + times + '" class="xubox_shade" style="'+ shadeStyle + ($(".xubox_shade").length>0 ? "display:none": "") +'"></div>');
    ...
    //own 取消最小化按钮 <a class="xubox_min" href="javascript:;"><cite></cite></a>
...
Class.pt.creat = function(){
    ...
    switch(config.type){
        ...
        case 1:
            if(page.html !== ''){
                ...
                body.append($(space[1]));
                $.post(page.url, function(datas){
                    $('#xuboxPageHtml'+ times).html(datas.toString());
                    page.ok && page.ok($('#'+ doms[0] + times) ,datas);
                    $('#'+ doms[0] + times).find(".xubox_yes").prop("disabled", false);
                });
            } else {
    ...
    if(config.title && (config.type !== 3 || config.type !== 4)){
        ...
        /*这是在原有组件上拓展的button,使用时注意避免与原组件的冲突*/
        for(var i = 0; i < confbtn.btn.length; i++){
            var btnList = [];
                btnList[i] = $('<input type="button" data-btn="xubox_btn'+ i +'" value="'+ confbtn.btn[i].text
                                +'" class="xubox_'+ confbtn.btn[i].type +' xubox_botton_'+  (confbtn.btn[i].type=== "yes"  ? "p" : "b")+'">');

                (function(i, $){
                    btnList[i].on("click", function(){
                        var validation = true;// 表单校验
                        if(confbtn.btn[i].validation){
                        	layerE.find("form").each(function(j, ele){ //遍历表单
    			            	if(!$(ele).valid()){
    			            		validation = false;
    			            	}
    			            })
					    ...
					    var flag = confbtn.btn[i].fn.call(this,layerE)
                        if(typeof(flag) == 'undefined'){ //关闭
                           confbtn.btn[i].close && layer.close(that.index);
                        }else if(flag){
            ...
            btnList[i].appendTo(layerBtn);
        }
    }

	if(config.page.url.length > 0){
		layerE.find(".xubox_yes").prop("disabled",true);
	}
    ...
    switch(config.type){
        case 0:
            ...
            if(config.title){
                ...
                layerE.find('.xubox_msgico').css({top: 11});
                layerE.find(doms[3]).css({marginTop : 13});
            }
        break;
        ...
        case 4:
            ...
            layerE.css({left: fowo.tipLeft-15, top: fowo.tipTop});
        break;
    };
...
Class.pt.autoArea = function(times){
    ...
    if(config.area[0] === 'auto'){
        var maxWidth = parseInt(config.maxWidth),
            layerPWid =layerPage.width(),
            layerElWid = layerE.width();

        if(layerPWid >= maxWidth){
            layerE.width(maxWidth);
        }else if(layerPWid > layerElWid && layerPWid < maxWidth){
            layerE.width(layerPWid);
        }
        ...
        layerE.css({width: config.area[0]});
    }

    //设置高度
    switch(config.type){
        case 0:
            var aBtn = layerE.find('.xubox_botton>input');
        ...
        case 1:
            ...
            btnHeight = config.btn.length > 0 ? 44 : 0;
            layerPage.css("width", "100%");

            if(page.html !== '' || page.url !== ''){ // 弹窗内容非页面dom元素
                if(config.area[1] !== "auto") { //非自适应
                    outHeight = parseInt(config.area[1]);
                }else {
                    outHeight =layerPage.outerHeight() > $(document).height() ? $(window).height()*0.9 :layerPage.height() + titHeight + btnHeight + 10; //是否超出窗口高度
                }
            }
        break;
        case 2:
    ...
    layerMian.css({height: outHeight});
    layerPage && layerPage.css({height: outHeight - titHeight - btnHeight});

    if(layerPage[0] && layerPage[0].scrollHeight > layerPage[0].clientHeight){
        layerE.width(layerE.width() + 17);
    }
...
Class.pt.move = function(){
    ...
    movedom.data("config", config);
    movedom.on('mousedown', function(M){
        if($(this).attr('move') === 'ok'){
            currentMoveObj = movedom;
            conf.ismove = true;
    ...
    if(conf.moveEvent) return; //设置一个标记避免多个弹出框时重复绑定事件
    $(document).mousemove(function(M){
        ...
        try{
            if(conf.ismove){
                conf.moveLayer();
                conf.move.remove();
                config.moveEnd && config.moveEnd(that.layerE);
                layer.topIndex(that.layerE);
            }
            conf.ismove = false;
        }catch(e){
            conf.ismove = false;
...
//     //扩展按钮
//     layerE.find('.xubox_extend1').on('click',function(){
//         config.extend1 ? config.extend1(that.index) : dialog.extend1(that.index);
// //        layer.close(that.index);
//     });
//     layerE.find('.xubox_extend2').on('click',function(){
//         config.extend2 ? config.extend2(that.index) : dialog.extend2(that.index);
// //        layer.close(that.index);
//     });
//     layerE.find('.xubox_extend3').on('click',function(){
//         config.extend3 ? config.extend3(that.index) : dialog.extend3(that.index);
// //        layer.close(that.index);
//     });

    if(that.config.shadeClose){
        $('.xubox_shade').on('click', function(){
            layer.close(that.index);
        });
    }
    ...
    layerE.find('.xubox_min').on('click', function(){
        ...
        layer.min.apply(this,[that.index, config])
    ...
    layerE.find('.xubox_max').on('click', function(){
        if($(this).hasClass('xubox_maxmin')){
            layer.restore(that.index,$(this));
            config.restore && config.restore(layerE);
        } else {
            layer.full(that.index, config,$(this));
...
//重置层
layer.area = function(index, options,_this){
    ...
    if(type === ready.type[1] || type === ready.type[2]){
        ...
        //计算大小（own）
        var p_w = 0,p_h = 0;
            p_w = _this.parents(".xubox_main").width();
            p_h = _this.parents(".xubox_main").height();
            _this.parents(".xubox_main").find(".xubox_page").css({"width":p_w ,"height":p_h-(parseInt(xubox_title_h)+parseInt(xubox_botton_h))});
    }
};
...
layer.min = function(index, options){
    ...
    layer.area(index, {width: 180, height: 35},$(_this));
    layero.find('.xubox_min').hide();
    layero.attr('type') === 'page' && layero.find(doms[4]).hide();
    ready.rescollbar(index);
    layero.find('.xubox_botton').hide();
};

//还原
layer.restore = function(index,_this){
    ...
    ready.rescollbar(index);
    layero.find('.xubox_botton').show();
};

//全屏
layer.full = function(index, options,_this){
    ...
    timer = setTimeout(function(){
        ...
        layer.area(index, {
            top: layero.css('position') === 'fixed' ? 5 : win.scrollTop(),
            left: layero.css('position') === 'fixed' ? 5 : win.scrollLeft(),
            width: win.width() - borders,
            height: win.height() - borders
       },_this);
    }, 100);
...
layer.close = function(index){
    //修改close函数的参数，使其支持传入$(dom)来关闭弹窗 2015/11/22 hbf
    var layero , type , shadeNow;
    if(typeof(arguments[0]) == "object" ){//参数传入的是弹窗dom元素
        layero = $(arguments[0]);
        var _arr = layero.attr("id").split(doms[0]);
        index = _arr[_arr.length-1];
    }else{
        layero = $('#'+ doms[0] + index)
    }
    type = layero.attr('type'),shadeNow = $('#xubox_moves, #xubox_shade' + index);
//    var layero = $('#'+ doms[0] + index), type = layero.attr('type'), shadeNow = $('#xubox_moves, #xubox_shade' + index);
    ...
    if(shadeNow.css("display") === "none"){
        shadeNow.remove();
    }
    if($(".xubox_layer").length < 1){
        $(".xubox_shade").remove();
    }
...
layer.topIndex = function(layero){ //人工置顶！！！
    var layerArry = $(".xubox_layer");
    var zIndexArry = [];

    if(layerArry.length>1){
        for (var i = 0; i < layerArry.length; i++) {
            zIndexArry.push($(layerArry[i]).css("z-index"));
        }
        if(layero.css("z-index") < Math.max.apply(null, zIndexArry)){
            layero.css("z-index", Math.max.apply(null, zIndexArry) + 1);
        }
    }
}

//设置弹窗层级 add by hbf 2016/04/16
layer.setZindex = function(index,zIdx){
   var layero = $('#'+ doms[0] + index);
   var shade = $('#xubox_shade'+ index);
   layero.css("z-index",zIdx);
   shade.css("z-index",layero.css("z-index")-1);
}
